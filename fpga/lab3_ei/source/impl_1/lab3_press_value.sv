...
module lab3_press_value (
    ...
    lab3_presslogic (.clk(clk), .nreset(nreset), .enable(row_sync[0]), .d_in(~col_sync), .q_out({key[3:1], key[10]});
	
	// sequential logic block for a hardware flip-flop register
    lab3_presslogic (.clk(clk), .nreset(nreset), .enable(row_sync[1]), .d_in(~col_sync), .q_out({key[6:4], key[11]});
	
	// sequential logic block for a hardware flip-flop register
    lab3_presslogic (.clk(clk), .nreset(nreset), .enable(row_sync[2]), .d_in(~col_sync), .q_out({key[9:7], key[12]});
	
	// sequential logic block for a hardware flip-flop register
    lab3_presslogic (.clk(clk), .nreset(nreset), .enable(row_sync[3]), .d_in(~col_sync), .q_out({key[15:13], key[0]}));
