...
// check that all 4 output transitions work	
    ...
    nreset = 0;
        assert (row == 4'b0000) begin      // check outputs
	...
	#6_000_000; // waiting till the count of 5_999_999 - doing half of 120_000_000 because it'll still be at this row value at any point below 120_000_000. 
	// This logic will work for the others because as long as they're below the count it's supposed to reach the row logic will be sustained.
	...
	#120_000_000; // waiting till the count of 11_999_999
	...
	#120_000_000; // waiting till the count of 5_999_999

			assert (row == 4'b0010) begin       // check outputs
				$display("PASSED! The output transitions (rows) behaves as desired at time: %0t.", $time);
			end else begin
				$error("FAILED! The output transitions (rows) behaves incorrectly at time: %0t.", $time); 
			end
	...
	#120_000_000; // waiting till the count of 11_999_999
			...
			assert (row == 4'b0001) begin       // check outputs
				$display("PASSED! The output transitions (rows) behaves as desired at time: %0t.", $time);
			end else begin
				$error("FAILED! The output transitions (rows) behaves incorrectly at time: %0t.", $time); 
			end
